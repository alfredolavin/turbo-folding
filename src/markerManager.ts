...
export const DEFAULT_VIVID_PALETTE: string[] = [
    '#FF3366', // Neon Pink
...
export function createSvgDataUri(color: string): vscode.Uri {
...
export class MarkerManager implements vscode.Disposable {
    // Map of hex color -> TextEditorDecorationType
    private decorationTypes: Map<string, vscode.TextEditorDecorationType> = new Map();
    // Map document URI string -> Map of (line number -> MarkerItem)
    private markers: Map<string, Map<number, MarkerItem>> = new Map();
    ...
    private _onDidChangeMarkers = new vscode.EventEmitter<vscode.Uri | undefined>();
    public readonly onDidChangeMarkers = this._onDidChangeMarkers.event;
    ...
    public getPalette(): string[] {
        ...
        const customPalette = config.get<string[]>('palette');
        if (Array.isArray(customPalette) && customPalette.length > 0) {
            return customPalette;
        }
        return DEFAULT_VIVID_PALETTE;
    }

    public getColorAssignmentStrategy(): 'sequential' | 'random' {
        const config = vscode.workspace.getConfiguration('turboFolding');
        return config.get<'sequential' | 'random'>('colorAssignment', 'sequential');
    }
    ...
    private assignColor(docUri: string): string {
    ...
    private loadMarkers() {
        const saved = this.context.workspaceState.get<Record<string, Array<any>>>('turboFolding.savedMarkers', {});
        const palette = this.getPalette();

        for (const [uriStr, items] of Object.entries(saved)) {
            if (Array.isArray(items) && items.length > 0) {
                const markerMap = new Map<number, MarkerItem>();
                let idx = 0;
    ...
    private async saveMarkers() {
        const data: Record<string, MarkerItem[]> = {};
    ...
    public hasMarker(docUriOrEditor: string | vscode.TextEditor, line: number): boolean {
    ...
    public addMarker(editor: vscode.TextEditor, lineNumber?: number): boolean {
    ...
    public deleteMarker(docUriOrEditor: string | vscode.TextEditor, line: number) {
        const docUri = typeof docUriOrEditor === 'string' ? docUriOrEditor : docUriOrEditor.document.uri.toString();
        const markerMap = this.markers.get(docUri);
    ...
    public getMarkers(documentUri: vscode.Uri | string): MarkerItem[] {
    ...
    public reanchorMarkers(document: vscode.TextDocument): boolean {
    ...
    public handleDocumentChange(event: vscode.TextDocumentChangeEvent) {
        ...
        let currentMarkers: MarkerItem[] = Array.from(markerMap.values());
    ...
    public updateDecorations(editor: vscode.TextEditor, doReanchor: boolean = false) {
        if (doReanchor) {
