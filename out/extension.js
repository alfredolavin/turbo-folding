...
const foldingTreeView_1 = require("./foldingTreeView");
function activate(context) {
    ...
    const toggleFocusOnSelectCmd = vscode.commands.registerCommand('turboFolding.toggleFocusOnSelect', async () => {
        ...
        updateViewDescription();
        vscode.window.showInformationMessage(`Turbo Folding: Focus on Select is now ${focusOnSelect ? 'ENABLED (Select marker to unfold & fold others)' : 'DISABLED'}`);
    });
    // Helper to focus a specific marked line: unfolds target and folds all other marked blocks
    async function focusMarkedLine(editor, targetLine) {
    ...
    const selectMarkerFromViewCmd = vscode.commands.registerCommand('turboFolding.selectMarkerFromView', async (uri, line) => {
    ...
    const focusMarkerItemCmd = vscode.commands.registerCommand('turboFolding.focusMarkerItem', async (item) => {
    ...
    const deleteMarkerItemCmd = vscode.commands.registerCommand('turboFolding.deleteMarkerItem', (item) => {
        if (item && item.documentUri && item.line !== undefined) {
    ...
    const addMarkersAtSameLevelCmd = vscode.commands.registerCommand('turboFolding.addMarkersAtSameLevel', async (itemOrArgs) => {
        ...
        const rawSymbols = await (0, foldingManager_1.getDocumentSymbols)(editor.document);
        const symbols = (0, foldingManager_1.flattenSymbols)(rawSymbols);
        const refSemantic = (0, foldingManager_1.resolveSemanticInfo)(editor.document, referenceLine, symbols);
        const matchingLines = await (0, foldingManager_1.findFoldablesAtSameLevel)(editor.document, referenceLine, true);
        let added = 0;
        ...
        if (added > 0) {
            vscode.window.showInformationMessage(`Turbo Folding: Added ${added} marker${added !== 1 ? 's' : ''}${typeDesc} for ${matchingLines.length} foldable${matchingLines.length !== 1 ? 's' : ''} at same level${alreadyMarked > 0 ? ` (${alreadyMarked} already existed)` : ''}.`);
        }
        else {
            vscode.window.showInformationMessage(`Turbo Folding: All ${matchingLines.length} foldable${matchingLines.length !== 1 ? 's' : ''}${typeDesc} at this level are already marked.`);
        }
    });
